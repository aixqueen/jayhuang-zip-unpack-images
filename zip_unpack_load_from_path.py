...
def _resolve_any_path(path_value: str) -> Path:
    """
    Accept absolute path or relative path (relative to ComfyUI input/).
    """
    if not path_value or not str(path_value).strip():
        raise ValueError("zip_path is empty. Connect output zip_path from 'Zip Upload Only (.zip)'.")
...
class ZipUnpackLoadFromPath:
    """
    zip_path (STRING) -> unzip -> load images -> IMAGE batch
    """
    ...
    def run(
        ...
        images = torch.cat(batch_tensors, dim=0)  # B,H,W,3
